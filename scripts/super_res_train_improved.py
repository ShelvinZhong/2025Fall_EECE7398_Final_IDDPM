...
def load_superres_data(hr_dir, lr_dir, batch_size, large_size, small_size, deterministic=False):
...
def evaluate_metrics(
    ...
    for i, (hr_batch, model_kwargs) in enumerate(val_loader):
...
def main():
    ...
    out_dir = cfg.get("out_dir", "./sr_checkpoints_improved")
    ...
    logger.log("Experiment Mode: Improved Model (Paired HR/LR)")
    logger.log(
        f"LR Schedule: Base {lr:.1e}, Warmup {warmup_steps}, "
        f"Decay every {lr_decay_steps} by {lr_gamma}"
    )

    model_diffusion_defaults = sr_improved_model_and_diffusion_defaults()
    model_diffusion_args = {
        k: (cfg[k] if k in cfg else v_default)
    ...
    for step in pbar:
        ...
        if (step + 1) % log_interval == 0:
            logger.log(f"step {step+1}, loss {loss.item():.6f}, lr {cur_lr:.6e}")

        if (step + 1) % eval_interval == 0:
            logger.log(f"Starting evaluation at step {step+1}...")
            ema_model, _ = sr_improved_create_model_and_diffusion(**model_diffusion_args)
            ...
            logger.log(
                "Evaluation Step {} | PSNR: {:.2f} | SSIM: {:.4f} | LPIPS: {:.4f} | "
                "NIQE: {:.4f} | PI: {:.4f} | CLIPIQA: {:.4f} | MUSIQ: {:.4f}".format(
                    step + 1,
                    metrics["psnr"],
                    metrics["ssim"],
                    metrics["lpips"],
